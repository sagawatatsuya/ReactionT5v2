...
if __name__ == "__main__":
    ...
    for i in range(0, query_embedding.shape[0], batch_size):
        ...
        _, nearest_sample = torch.max(similarity, dim=1)
        nearest_samples.append(nearest_sample.cpu().tolist())

    nearest_samples = set(np.concatenate(nearest_samples))
    
    df = pd.read_csv(config.input_data)
    df = df.iloc[list(nearest_samples)]
    df.to_csv(os.path.join(config.output_dir, "similar_reactions.csv"), index=False)